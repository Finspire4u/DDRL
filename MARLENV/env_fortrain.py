...
class EnvDrones(object):
    def __init__(self, num, SR_rate):
        ...
        self.datasize = 24000000000 # (30000Gb) total pkts in transmission (can be fully tans under ideal scenario)
        ...
        for _ in range(self.agents-1):
            ...
            # self.action_space.append(MultiDiscrete([[0,4], [0,4]])) # First [0,4] coverage, Third [0,4] rate
...
####################
    # To assign and initialize agent's or background's beam
    ...
    # To change agent's facing angle or background's beam
    def Antenna_update(self, nodes_position, pairs, antenna_coverage, max_half_coverage):
        # For agents
        if pairs.any():
            for i in range(len(pairs)):
                x1, y1 = nodes_position[pairs[i,0],0], nodes_position[pairs[i,0],1]
                x2, y2 = nodes_position[pairs[i,1],0], nodes_position[pairs[i,1],1]
    ...
    # To calculate background's beam distance by beamwidth
...
####################
    ...
    # To find all side lobes for all nodes
...
####################
    ...
    #         for x in range(3):
    #             for y in range(3):
    #                 obs[x,y] = self.grid_array[next_drone.pos[0]+x-1][next_drone.pos[1]+y-1]
    #         return obs
    #     return obs
    ...
    def _set_facing_angle(self, num, old_angle):
        if old_angle >= 90:
            if num == 1:
                ...
                return min(180, old_angle + 5)
            else:
                return min(180, old_angle + 10)
        else:
            if num == 1:
                return max(0, old_angle - 10)
            elif num == 2:
                return max(0, old_angle - 5)
            elif num == 3:
                return min(89, old_angle + 5)
            else:
                return min(89, old_angle + 10)

    # Based on actions, return half beamwidth and beam distance of agents
