import os
from env_addbgd import EnvDrones
import random
...
def test(num_nodes, bgd_step):
    ...
    writer = pd.ExcelWriter('./output/DEL_addbgd.xlsx')
    pd.DataFrame(DEL).to_excel(writer, 'page_1', float_format = '%0.2f')
    writer.save()
    writer.close()

    writer = pd.ExcelWriter('./output/THR_addbgd.xlsx')
    pd.DataFrame(THR).to_excel(writer, 'page_1', float_format = '%0.2f')
    writer.save()
    writer.close()

    writer = pd.ExcelWriter('./output/LST_addbgd.xlsx')
